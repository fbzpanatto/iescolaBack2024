...
exports.QuestionRouter.get('/form', (req, res) => {
    question_1.questionController.questionForm(req)
        .then(r => res.status(r.status).json(r))
        .catch(e => res.status(e.status).json(e));
});
exports.QuestionRouter.get('/owner/:id', (req, res) => {
    question_1.questionController.isOwner(req)
        .then(r => res.status(r.status).json(r))
        .catch(e => res.status(e.status).json(e));
});
