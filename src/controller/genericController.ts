...
import {helperDuplicatedStudents, helperStudentQuestions} from "../utils/helpers";
...
export class GenericController<T> {
  ...
  async getStudentsQuestionsSql(test: Test, testQuestions: TestQuestion[], classroomId: number, yearName: string, studentClassroomId: number | null) {
    ...
    `;
      ...
      return helperStudentQuestions(test, rows as Array<{[key:string]:any}>)
    }
  ...
  async qStudentClassroomsForTest(
    test: Test,
    classroomId: number,
    yearName: string,
    studentClassroomId: number | null
  ...
  async qTestQuestionsForMultipleTests(
    testIds: number[]
  ) {
  ...
  async updateTeacherContractCurrentYear(
    body: {
      teacherId: number,
      schoolId: number,
      contractId: number,
