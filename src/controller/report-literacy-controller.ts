...
import { iLocalSchool } from "../interfaces/interfaces";
  ...
  async getReport(request: Request) {
    ...
    try {
      ...
      return await AppDataSource.transaction(async(CONN) => {
        ...
        const allSchools = await CONN.getRepository(School)
        ...
        const filteredData = await CONN.getRepository(School)
          .createQueryBuilder("school")
          .leftJoinAndSelect("school.classrooms", "classroom")
          .leftJoinAndSelect("classroom.studentClassrooms", "studentClassrooms")
          .leftJoinAndSelect("studentClassrooms.year", "year")
        ...
        const cityHall = {
          id: "ITA", name: "PREFEITURA DO MUNICIPIO DE ITATIBA", shortName: "PREFEITURA DO MUNICIPIO DE ITATIBA",
        ...
        for (let school of finalArray) {
          let localSchool: iLocalSchool = { id: school.id, name: school.name, tiers: [] }
...
export const literacyReportController = new ReportLiteracy();
