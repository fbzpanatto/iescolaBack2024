...
ClassroomRouter.get('/', havePermission, REQUEST_CLASSROOM_QUERY, async (req: Request, res: any) => { const response = await controller.getAllClassrooms(req, (req as any).user); return res.status(response.status).json(response)})
...
ClassroomRouter.put('/:id', havePermission, ID_PARAM, async (req: Request, res: any) => { const response = await controller.putClassroomAndStudents(req, (req as any).user); return res.status(response.status).json(response)})