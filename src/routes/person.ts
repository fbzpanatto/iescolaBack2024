...
PersonRouter.get('/:id', ID_PARAM, async (req: Request<{ id: number | string }>, res: any) => { const response = await controller.findOneById(req.params.id, req); return res.status(response.status as number).json(response) })
...
PersonRouter.put('/:id', ID_PARAM, async (req: Request<{ id: number | string }>, res: any) => { const response = await controller.updateId(req.params.id, req.body); return res.status(response.status as number).json(response) });
