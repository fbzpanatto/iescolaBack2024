...
StudentRouter.get("/:year/all", PARAM_YEAR, havePermission, async (req: Request, res: Response) => {
    const response = await studentController.allStudents(req); return res.status(response.status as number).json(response);
  },
);

StudentRouter.get("/:id", PARAM_ID, havePermission, async (req: Request, res: Response) => {
    const response = await studentController.findOneStudentById(req); return res.status(response.status).json(response);
  },
);

StudentRouter.post("/inactive", havePermission, async (req: Request, res: Response) => {
  const response = await studentController.inactiveNewClass(req.body); return res.status(response.status).json(response);
})

StudentRouter.post("/", ...CREATE_VALIDATORS, havePermission, async (req: Request, res: Response) => {
    const response = await studentController.save(req.body); return res.status(response.status).json(response);
  }
)

StudentRouter.put("/literacy-first/:id", PARAM_ID, havePermission, async (req: Request, res: Response) => {
    const response = await studentController.putLiteracyBeforeLevel(req.body); return res.status(response.status).json(response);
  }
)

StudentRouter.put("/:id/graduate", PARAM_ID, havePermission, async (req: Request, res: Response) => {
  const response = await studentController.graduate(req.params.id, req.body); return res.status(response.status).json(response);
})

StudentRouter.put("/:id", ...UPDATE_VALIDATORS, havePermission, async (req: Request, res: Response) => {
    const response = await studentController.updateId(req.params.id, req.body); return res.status(response.status).json(response);
  },
)